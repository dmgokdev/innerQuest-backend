...
import { PLAN_NOT_FOUND } from '../constants';
...
export class PlanService {
	...
	async getAllPlans() {
		...
		const totalCount = await prisma.plans.count(options);
		...
		const allRecords = await prisma.plans.findMany(options);

		if (!allRecords || !Array.isArray(allRecords) || allRecords.length === 0)
			throw new AppError(PLAN_NOT_FOUND, HttpStatus.NOT_FOUND, allRecords);
	...
	async getPlan() {
		...
		if (!record || !record.id)
			throw new AppError(PLAN_NOT_FOUND, HttpStatus.NOT_FOUND);
		return record;
	}
	...
	async updatePlan() {
		...
		const updateRecord = await prisma.plans.update({
			where: {
				deleted: false,
				id: parseInt(id, 10),
			},
